...
const Table = () => {
    ...
    const renderHeader = () => {
        ...
        return headerElement.map((key, index) => {
            return <th className=" mx-5 text-xxs lg:text-xl lg:px-4" key={index}>{key.toUpperCase()}</th>
        })
    }
    ...
    const renderBody = () => {
        return tabelleBL && tabelleBL.map(({ position, emblem, team, playedGames, won, lost, draw, goalsFor,goalsAgainst, goalDifference, points }) => {
            return (
                <tr className="mb-auto text-xs text-gray-100 bg-gray-800 lg:font-extrabold lg:text-xl"  key={position}>
                    <td>{position}</td>
